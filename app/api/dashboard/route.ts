...
export async function GET() {
  ...
  const pipelineValue = deals.reduce((sum: number, d: { value: number }) => sum + Number(d.value), 0);
  const mrr = activeClients.reduce((sum: number, c: { mrr: number }) => sum + Number(c.mrr), 0);
  ...
  return NextResponse.json({
    kpis: {
      newLeads: leadsRes.rows[0].count,
      pipelineValue,
      activeClients: activeClients.length,
      mrr,
      pendingTasks: tasksRes.rows[0].count,
    },
    stageBreakdown,
