...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const extraSets = isStageChange ? `, "stageMovedAt" = NOW()` : "";
  const sets = fields.map((f, i) => `"${f}" = $${i + 2}`).join(", ");
  const values = fields.map((f) => body[f]);

  const { rows } = await db.query(`
    WITH upd AS (
      UPDATE "Deal" SET ${sets}${extraSets}, "updatedAt" = NOW() WHERE id = $1 RETURNING *
    )
    SELECT u2.*,
  ...
  if (isStageChange) {
    await db.query(`
      ...
      VALUES (gen_random_uuid()::text, 'deal_moved', $1, $2, $3, NOW())
    `, [`Deal movido a "${body.stage}"`, id, session.user?.id ?? "system"]);

    if (body.stage === "Cerrado ganado") {
      const clientCheck = await db.query(`SELECT id FROM "Client" WHERE "dealId" = $1 LIMIT 1`, [id]);
      if (!clientCheck.rows[0]) {
        const contactName = existing.rows[0].leadId
          ...
          : null;
        await db.query(`
          INSERT INTO "Client" (id, company, contact, "dealId", status, plan, mrr, "createdAt", "updatedAt")
          VALUES (gen_random_uuid()::text, $1, $2, $3, 'Activo', 'Starter', 0, NOW(), NOW())
