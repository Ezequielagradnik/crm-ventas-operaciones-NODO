...
export async function GET(req: NextRequest) {
  ...
  const [leads, deals, clients, tasks] = await Promise.all([
    db.query(
      `SELECT id, name, company FROM "Lead" WHERE name ILIKE $1 OR company ILIKE $1 OR email ILIKE $1 LIMIT 5`,
      [pattern]
    ),
    db.query(
      `SELECT id, company, stage FROM "Deal" WHERE company ILIKE $1 LIMIT 5`,
      [pattern]
    ),
    db.query(
      `SELECT id, company, contact FROM "Client" WHERE company ILIKE $1 OR contact ILIKE $1 LIMIT 5`,
      [pattern]
    ),
    db.query(
      `SELECT id, title, status FROM "Task" WHERE title ILIKE $1 LIMIT 5`,
      [pattern]
    ),
  ]);
