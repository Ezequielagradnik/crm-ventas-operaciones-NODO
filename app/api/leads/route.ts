...
export async function GET(req: NextRequest) {
  ...
  const { rows } = await db.query(`
    ...
    FROM "Lead" l LEFT JOIN "User" u ON l."ownerId" = u.id
    ${where} ORDER BY l."createdAt" DESC
  `, params);
...
export async function POST(req: NextRequest) {
  ...
  `, [name, company??null, email??null, phone??null, linkedin??null, origin??"Otro", vertical??"Otro", status??"Nuevo", notes??null, resolvedOwnerId]);

  const lead = rows[0];
  await db.query(`INSERT INTO "Activity" (id, type, message, "leadId", "userId", "createdAt") VALUES (gen_random_uuid()::text, 'lead_created', $1, $2, $3, NOW())`,
