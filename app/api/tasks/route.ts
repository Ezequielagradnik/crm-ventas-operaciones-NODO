...
export async function POST(req: NextRequest) {
  ...
  const { rows } = await db.query(`
    ...
    LEFT JOIN "Deal" d ON i."dealId" = d.id
  `, [title, description ?? null, priority ?? "Media", status ?? "Pendiente",
      dueDate ? new Date(dueDate) : null, resolvedOwnerId, leadId ?? null, dealId ?? null]);
